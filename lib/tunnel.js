...
class DeviceTunnel extends events.EventEmitter {
  ...
  bindAirplayServer() {
    ...
    server.sessions.on('new', this.handleClientConnected.bind(this))



    server.start({ name: this.deviceName, port: lastPort++ })

      // let targetVol = 100 - Math.floor(-1 * (Math.max(vol, -30) / 30) * 100);
      this.device.setVolume(50, EMPTY_CALLBACK);
  ...
  handleClientConnected(session) {
    ...
    this.icecastServer = new Nicercast(session, {
